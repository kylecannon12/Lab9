...
class SudokuGenerator:
    ...
    def is_valid(self, row, col, num):
        return (self.valid_in_col(row, num) and
                self.valid_in_row(col, num) and
    ...
    def remove_cells(self):
        cells_to_remove = random.sample(range(self.row_length ** 2), self.removed_cells)
        #creates list of n random unique ints in the range of the total grid
        # n is removed_cells

        for cell in cells_to_remove:
            row = cell // self.row_length
            col = cell % self.row_length
            self.board[row][col] = 0
